...
class MenuItemTableViewCell: UITableViewCell {
    ...
    func setup(type: MenuItemType, with model: MeDataModel?){
        ...
        case .disease:
            ...
            self.subtitleLabel.text = model?.me.access.diagnosisTotal == nil ? "\(model?.me.access.diagnosisUsed ?? 0)/\(model?.me.access.diagnosisUsed ?? 0)" : "\(model?.me.access.diagnosisUsed ?? 0)/\(model?.me.access.diagnosisTotal ?? 0)"
            self.accessoryType = .disclosureIndicator
        case .lang:
